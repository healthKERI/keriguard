...
class Issuer:
    ...
    async def issue_connection_credential(
        ...
        creder: Optional[serdering.SerderACDC] = None
        ...
        creder2: Optional[serdering.SerderACDC] = None
        for saider in saiders:
            (
                c,
                *_,
    ...
    async def revoke_connection_credential(
        ...
        """
        Revoke a previously issued connection credential.
        ...
        try:
            ...
            # Serialize for publishing: registry inception + revocation TEL event + anchoring event
            anc = self.hab.db.cloneEvtMsg(aserder.pre, 0, aserder.said)
            reg = self.rgy.reger.cloneTvtAt(creder.regi)
            rev = self.rgy.reger.cloneTvtAt(creder.said, sn=1)
